...
function buildModal(mood: string, slackUserId: string) {
  return {
    ...
    blocks: [
      {
        type: "section",
        text: { type: "mrkdwn", text: `Humeur choisie : *${MOOD_LABELS[mood]}*` },
      },
      { type: "divider" },
      {
...
export async function POST(req: NextRequest) {
  ...
  if (payload.type === "block_actions") {
    ...
    // Ouvrir la modale sans attendre les vérifications DB
    ...
    // Répondre à Slack immédiatement
    return new NextResponse("ok")
  }

  // ── Soumission modale → vérifier DB et sauvegarder ───────────────────────
  if (payload.type === "view_submission") {
    ...
    if (!user) {
      return NextResponse.json({
        ...
        errors: { context_block: "Compte non lié à Pulse. Connecte-toi une fois sur l'app." },
      })
    }
    ...
    if (existing) {
      return NextResponse.json({
        ...
        view: {
          ...
          blocks: [{
            type: "section",
            text: { type: "mrkdwn", text: `Tu as déjà partagé ta météo aujourd'hui : *${MOOD_LABELS[existing.mood]}*` },
          }],
        },
      })
    }
    ...
    // Poster dans Slack si partagé avec l'équipe
